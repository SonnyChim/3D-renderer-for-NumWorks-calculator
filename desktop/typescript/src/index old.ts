...
class renderer {
    ...
    private offsetCache: Offset | undefined;
    private sortingTolerance: number;
    private screen!: ProjectionScreen;
    private triangleCoords: number[][][] = [];
    private materialIndeces: any = [];
    private materials: {} = {};
    scene: any;
    ...
    drawTri2d(tri2d: Tri2d, color: string) {
    ...
    drawTri3d(tri3d: Tri3d, color: string) {
        const tri2d: Tri2d = {};
    ...
    loadFile(e) {
        ...
        return new Promise<[string,string]>((resolve) => {
            if (file.name.slice(-4) === ".obj") {
                reader.addEventListener("load", () => { resolve(["obj", reader.result as string]); });
            } else if (file.name.slice(-4) === ".mtl") {
                reader.addEventListener("load", () => { resolve(["mtl", reader.result as string]); });
            }
            reader.readAsText(file);
        })
    }
    processFile(file: string, type) {
        if (type === "obj") {
            // .obj file
            // scale: 100
            // forward axis: y
            ...
            const vertices = file.split("\n")
                                 .filter((line) => { return line.slice(0, 2) === "v " }) // vertex
                                 .map((line) => {
                                    return line.split(/\s+/) // whitespace
                                               .slice(1, 4) // xyz
                                               .map((val) => { return parseFloat(val) });
                                });
            const faces = file.split("\n")
                              .filter((line) => { return line.slice(0, 2) === "f " }) // face
                              .map((line) => { // format: 1/1/1 2/2/2 3/3/3
                                return line.split(/\s+/) // whitespace
                                           .filter((val) => { return val }) // remove empty strings
                                           .slice(1) // vertices
                                           .map((val) => { return parseInt(val.split("/")[0]) }); // get first number
                                        })
            ...
            let index = 0;
            file.split("\n").forEach((val) => {
                if (val.slice(0, 7) === "usemtl ") {
                    this.materialIndeces.push([index, val.slice(7)]) 
                } else if (val.slice(0, 2) === "f " ) {
                    ++index
                }
            });
